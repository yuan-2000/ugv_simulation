...
// 用于计算预设时间向量场的时变增益
void calculateKGain(double simulation_t, double& k_gain, double& dot_k_gain) {
    if (simulation_t <= t_p) {
...
// 计算向量场
void computeVectorField(double x, double y, double& vx, double& vy, double& e_norm,double k_gain) {
    // 路径误差计算
...
void computeControlLaw(double vx, double vy, double x, double y, double theta,
                        ...
                        double& k_gain, double& dot_k_gain) {
    // 误差计算
    double e = (x * x) / (a * a) + (y * y) / (b * b) - c;
    ...
    // 雅可比矩阵 J 计算
    double J1 = -(1 + k_gain) * k * 4 * x * y / (a * a * b * b);
    double J2 = -(1 + k_gain) * k * (4 * x * x / (a * a * a * a) + 2 * e / (a * a));
    double J3 = -(1 + k_gain) * k * (4 * y * y / (b * b * b * b) + 2 * e / (b * b));
    double J4 = -2 / (b * b);
    double J5 = 2 / (a * a);
    double J6 = -k * e * 2 * x / (a * a);
    double J7 = -k * e * 2 * y / (b * b);
    ...
    // 计算参考角速度 theta_dot_d
    Eigen::Vector3d dot_p(norm_X * std::cos(theta), norm_X * std::sin(theta), dot_k_gain);
    ...
    omega_u = theta_dot_d - k_theta * ((std::pow(std::abs(widehat_p_E_X), alpha_u) * sign(widehat_p_E_X)) /
                                       widehat_p_dot);
    // omega_u = theta_dot_d - k_theta * ((std::pow(std::abs(1 - widehat_p.dot(widehat_mathcal_X)), alpha_u) * sign(1 - widehat_p.dot(widehat_mathcal_X))) /
    //                                    widehat_p_E_X);
...
int main(int argc, char** argv) {
    ...
    while (ros::ok()) {
        ...
        // calculateKGain(simulation_t, k_gain, dot_k_gain);
        // 计算向量场
        computeVectorField(x, y, vx, vy, e_norm,k_gain);
        // 计算控制输入
