...
const double k = 1.0;               // 向量场增益
const double alpha = 0.9;           // 非线性增益
const double k_theta = 2.0;         // 偏航角控制增益
const double alpha_u = 0.9;        // 角速度非线性增益
const int PUBLISH_RATE = 100;        // 控制频率（Hz）
const double v_max = 2.0;           // 最大线速度
const double omega_max = 4.0;       // 最大角速度
...
//         ROS_WARN("Received invalid odom data: NaN detected!");
...
//         ROS_WARN("Received invalid IMU data: NaN detected!");
//         return;  // 如果IMU数据无效，则返回，不进行更新
//     }
//     tf::Quaternion q(
//         msg->orientation.x,
//         msg->orientation.y,
...
void computeControlLaw(double vx, double vy, double x, double y, double theta,
                        double& v_u, double& omega_u,ros::Publisher& error_pub,ros::Publisher& theta_pub) {
    // 误差计算
    double e = (x * x) / (a * a) + (y * y) / (b * b) - c;
    ...
    theta_msg.data = std::abs(std::sin(theta));  // 发布sin_theta的绝对值
    ...
    // omega_u = theta_dot_d - k_theta * widehat_p_E_X ;
...
int main(int argc, char** argv) {
    ...
    ros::Subscriber amcl_sub = nh.subscribe("/amcl_pose", 20, amclPoseCallback);
    ...
    // 添加一个发布器，用于发布误差e
    ros::Publisher error_pub = nh.advertise<std_msgs::Float64>("/tracking_error", 20);
    ros::Publisher theta_pub = nh.advertise<std_msgs::Float64>("/control_error", 20);
    ...
    while (ros::ok()) {
        ...
        // 计算控制输入
        computeControlLaw(vx, vy, x, y, theta, v_u, omega_u,error_pub,theta_pub);
        ...
        ROS_INFO("Computer Control: v_u: %.2f, omega_u: %.2f", v_u, omega_u);
